...
calles(avenida_albert_einstein_s5).
...
calles(boulevard_walter_thilo_deininger_s7).
...
calles(carretera_panamericana_s13).
calles(carretera_panamericana_s14).
...
calles(boulevard_los_proceres_s5).
...
calles(calle_las_azucenas_s1).

%% Avenida Rio Amazonas, 1 seccion
calles(avenida_rio_amazonas_s1).
